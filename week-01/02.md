...
### **5. Shell Scripting: Intermediate Concepts**

- **Objective**: Dive deeper into shell scripting to introduce more powerful features.
- **Concepts to Cover**:
  - **Functions**: Defining reusable blocks of code.
  - **Command-line Arguments**: Use `$1`, `$2`, etc. to capture arguments passed to the script.
  - **Exit Codes**: Use `exit` to terminate a script and return a status code.
  - **Case Statements**: A better alternative to multiple `if-else` blocks.

**Learning Task**:

- Write a script that takes command-line arguments and performs actions based on them.
- Write functions to modularize your script.
...
- Write a script that takes a filename as an argument, checks if it exists, and reports its file size.
